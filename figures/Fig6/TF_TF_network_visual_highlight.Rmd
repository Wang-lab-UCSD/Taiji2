...
get_graph <- function(id){
    ...
    ge3 <- ge %>% filter(from %in% id | to %in% id)
    ...
    V(g)$size <- ifelse(V(g)$name %in% id, 12, 7)
    # V(g)$label.cex <- 0.75 # part label
...
```{r}
get_graph(c('Jun','Batf'))
# save to plot
pdf('graph_Jun_Batf_highlight.pdf', width = 8, height = 8)
get_graph(c('Jun','Batf'))
dev.off()
```

# selected TFs' neighbors in different contexts
```{r}
# define function
get_graph <- function(id){
    ge1 <- igraph::as_data_frame(g1) %>% select(from, to, corr) %>% transform(from=pmin(from, to), to=pmax(from,to)) %>% mutate(edge.color='#7F6000')
